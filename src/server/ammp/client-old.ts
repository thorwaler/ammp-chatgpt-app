...
export class AMPPAPIClient {
  ...
  async authenticate(apiKey: string): Promise<TokenInfo> {
    try {
      console.log('[AMMP Auth] Attempting authentication with API key');
      console.log('[AMMP Auth] URL:', `${this.baseURL}/v1/token`);
      ...
      console.log('[AMMP Auth] Response headers:', Object.fromEntries(response.headers.entries()));

      if (!response.ok) {
        let errorDetails = '';
        try {
          const errorText = await response.text();
          console.log('[AMMP Auth] Error response body:', errorText);
          errorDetails = errorText;
        } catch (e) {
          console.log('[AMMP Auth] Could not parse error response');
        }
        ...
        if (response.status === 401) {
          ...
          throw new Error(`API endpoint not found (404). URL: ${this.baseURL}/v1/token. This may indicate the wrong base URL or the endpoint path has changed. Error: ${errorDetails}`);
        }
        
        throw new Error(`Authentication failed (${response.status}): ${response.statusText}. Details: ${errorDetails}`);
      }
  ...
  async getSites(): Promise<SitesResponse> {
    return this.request<SitesResponse>('/v1/sites');
  }
  ...
  async getEnergyData(request: EnergyDataRequest): Promise<EnergyDataResponse> {
    ...
    const endpoint = request.site_id
      ? `/v1/sites/${request.site_id}/energy?${params}`
      : `/v1/portfolio/energy?${params}`;
  ...
  async getAlerts(request: AlertsRequest = {}): Promise<AlertsResponse> {
    ...
    const endpoint = request.site_id
      ? `/v1/sites/${request.site_id}/alerts?${params}`
      : `/v1/alerts?${params}`;
  ...
  async getPerformance(request: PerformanceRequest): Promise<PerformanceResponse> {
    ...
    const endpoint = request.site_id
      ? `/v1/sites/${request.site_id}/performance?${params}`
      : `/v1/portfolio/performance?${params}`;
  ...
  async getDevices(siteId: string): Promise<DevicesResponse> {
    return this.request<DevicesResponse>(`/v1/sites/${siteId}/devices`);
  }
  ...
  async getWeatherData(
    ...
    return this.request<WeatherDataResponse>(
      `/v1/sites/${siteId}/weather?${params}`
    );
  }
